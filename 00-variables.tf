...
variable "tags" {
  ...
  type        = list(string)
  default = [
    "kubernetes.io/cluster/talos-cute = owned"
  ]
}
variable image {
  description = "The image to use for the Linode instances."
  type        = string
...
variable "control_plane" {
  ...
  type = object({
    instance_type      = optional(string, "g6-standard-4")
...
variable "subnet_id" {
  description = "ID of the subnet where to place the VMs."
  type        = string
  default = "geen"
}
...
variable "machinetag" {
  default     = "talos"
  description = "The machine tag to use for the Linode instances."
  type        = string
}
